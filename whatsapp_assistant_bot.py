...
class BotConfig(object):
	...
	def set_lastin_chat_message(self, msg, time_id):
		self.lastin_msg = msg
		self.lastin_msg_id = time_id

	def get_lastin_chat_message(self):
		return self.lastin_msg, self.lastin_msg_id
		
	def set_lastout_chat_message(self, msg, time_id):
		self.lastout_msg = msg
		self.lastout_msg_id = time_id

	def get_lastout_chat_message(self):
		return self.lastout_msg, self.lastout_msg_id
...
class Bot(object):
	...
	def poll_chat_in(self):
		lastin_msg = chat_history_in()
		if lastin_msg:
			time_id = time.strftime('%H-%M-%S', time.gmtime())
			last_saved_msg, last_saved_msg_id = self.config.get_lastin_chat_message()
			if last_saved_msg != lastin_msg and last_saved_msg_id != time_id:
				self.config.set_lastin_chat_message(msg=lastin_msg, time_id=time_id)
				
				print(self.config.get_lastin_chat_message())
				
				is_action = is_action_message(lastin_msg)
				if is_action:
					self.config.set_last_command(lastin_msg)
					self.bot_options(action=lastin_msg)
					
	def poll_chat_out(self):
		lastout_msg = chat_history_out()
		if lastout_msg:
			time_id = time.strftime('%H-%M-%S', time.gmtime())
			last_saved_msg, last_saved_msg_id = self.config.get_lastout_chat_message()
			if last_saved_msg != lastout_msg and last_saved_msg_id != time_id:
				self.config.set_lastout_chat_message(msg=lastout_msg, time_id=time_id)
				
				print(self.config.get_lastout_chat_message())
				
				is_action = is_action_message(lastout_msg)
				if is_action:
					self.config.set_last_command(lastout_msg)
					self.bot_options(action=lastout_msg)
